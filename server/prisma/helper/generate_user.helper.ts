...
import { generateRandomDate, generateRandomDigits, pickRandom } from './random_generator.helper';
import {
	userCities,
	userFirstNames,
	userIncomes,
...
export const generateUser = async (role?: UserRole, email?: string, password?: string, is_active?: boolean) => {
	return {
		...
		date_of_birth: generateRandomDate(new Date(1960, 0, 1), new Date(2005, 11, 31)),
		...
		id_card_issue: generateRandomDate(new Date(2015, 0, 1), new Date(2023, 11, 31)),
		id_card_expiry: generateRandomDate(new Date(2023, 12, 31), new Date(2020, 11, 31)),
